import { Component } from '@angular/core';
import { WeatherService } from './Weather.service';

// Classe AppComponent.
@Component({
  selector: 'app-root',
  templateUrl: './app.component.html',
  styleUrls: ['./app.component.css']
})
export class AppComponent {
dailyForecast: any; // / Propriété non utilisée, peut-être prévue pour une future utilisation.
  title(title: any) {
    throw new Error('Method not implemented.'); // / Méthode non implémentée, pourrait être supprimée si non utilisée.

  }
  city: string = ''; // Propriété pour stocker le nom de la ville entré par l'utilisateur.
  currentWeather: any; // Stocke les données météo actuelles récupérées.
  hourlyForecast: any; // Stocke les prévisions horaires récupérées.
  weeklyForecast: any; // Affecte les prévisions hebdomadaires à weeklyForecast.

  // Injecte le service WeatherService dans le constructeur.
  constructor(private weatherService: WeatherService) { }

  searchWeather() {
    this.weatherService.getWeatherByCity(this.city).subscribe(
      weatherData => {
        this.currentWeather = weatherData.current; // Stocke les données actuelles dans currentWeather.
        this.hourlyForecast = weatherData.hourly; // Stocke les prévisions horaires dans hourlyForecast.
        this.weeklyForecast = weatherData.daily; // Stocke les prévisions hebdomadaires dans weeklyForecast.
      },
      error => {
        console.error('Error fetching weather data', error);  // Affiche une erreur en cas d'échec de récupération des données météo.

      }
    );
  }


// Méthode pour afficher l'image correspondant à la description de la couverture d'air.
  getTemperatureIcon(description: string): string {
    switch (description.toLowerCase()) {
      case 'clear sky': // methode reparéee pour le ciel clair
      return 'https://media.istockphoto.com/id/1208368568/photo/dramatic-sunset-and-sunrise-sky-nature-background-with-white-clouds.jpg?s=612x612&w=0&k=20&c=mu12e5RAWbRKShGqpuRtNJHN7ZXHiohsmSBFbUXchT4=';
      case 'few clouds':

      case 'broken clouds': // methode réparée pour les nuages cassés.
      return'https://www.google.fr/url?sa=i&url=https%3A%2F%2Fwww.freeimages.com%2Fphoto%2Fbroken-clouds-1537880&psig=AOvVaw15tBu7YZRAwz3-Z7ZV2kxG&ust=1720828755011000&source=images&cd=vfe&opi=89978449&ved=0CBEQjRxqFwoTCKCxvuaYoIcDFQAAAAAdAAAAABAF';
      case 'overcast clouds': // methode repartition de la couverture d'air
      return 'data:image/png;base64,';
      case 'light rain':
      case 'moderate rain':

      case 'heavy intensity rain': // methode reparee pour l'image de pluie
      return 'https://www.shutterstock.com/image-vector/rain-icon-trendy-flat-style-600nw-421269391.jpg';
      case 'snow':
      case 'light snow': // methode reparee pour l'image de neige
      return 'https://images.unsplash.com/photo-1598347104365-2263f1870f8a?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D';
      case 'heavy snow': // methode reparee pour l'image de neige
      return 'https://as1.ftcdn.net/v2/jpg/01/99/63/42/1000_F_199634231_Ud905d27A72yRlueUhYHMVg6kuxrtmVE.jpg';
      default: // methode reparee pour l'image de pluie
      return 'https://www.shutterstock.com/image-vector/rain-icon-trendy-flat-style-600nw-421269391.jpg';
    }
  }
}
